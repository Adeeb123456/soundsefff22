...
class ProcessSound(contxt:Context?,repository: Repository,soundevent:ProcessSoundClick) {
companion object {
...
var context:Context? = null
    ...
    init {
        this.context=contxt
        repo=repository
        countSound=soundevent.pos
        this.soundCategory=soundevent.voiceItemModel!!.soundCategory
    ...
    fun initAndProcessSound(){
        ...
        when(soundCategory){
            ...
            SOUND_FRM_iMPORT->{
               ...
               importedSoundpath= Constants.importedSoundPath
   ...
   var callback: CommonRecycleAdapter.ProcessCallBak?=null
    fun initAndProcessSoundAdapter(calback1:CommonRecycleAdapter.ProcessCallBak){
this.callback=calback1
        when(soundCategory){
            SOUND_FRM_RECORD->{
                var path= getTempFile()
                soundType= SounType.SOUNDFROMRECORD
